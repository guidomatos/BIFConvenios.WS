...
#Region " Exportar DBF "
    ...
    Public Sub ExportaDBFUNICA(pData As DataSet, pNombreArchivoProceso As String)
        ...
        For Each ldr In pData.Tables(0).Rows
            Dim command As New OleDbCommand("INSERT INTO '" + pNombreArchivoProceso.Split(".")(0) +
            "' (documento, monto, tipo)" +
            " VALUES ( ?, ?, ?)",
            connection)
    ...
    Public Sub ExportaDBFSIGA(ByRef pData As DataSet, pNombreArchivoProceso As String)
        ...
        For Each ldr In pData.Tables(0).Rows
            ...
            'VALUES ( )
            Dim command As New OleDbCommand("INSERT INTO '" + pNombreArchivoProceso.Split(".")(0) +
            "' (depe_id, mes, anno, dni, sitlaboral, apellidos, nombres, monto, ncuota, maxcuota)" +
            " VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            connection)
    ...
    Public Sub ExportaDBFJAEN(pData As DataSet, pNombreArchivoProceso As String)
        ...
        For Each ldr In pData.Tables(0).Rows
            ...
            'VALUES ( )
            Dim command As New OleDbCommand("INSERT INTO '" + pNombreArchivoProceso.Split(".")(0) +
            "' ( PERIODO, EMPRESA, CODMOD, CARGO, CARBEN, T_PLANI, CODDES, MONTODES, APEPATER, APEMATER, NOMBRE)" +
            " VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            connection)
            ' " VALUES ( @depe_id, @mes, @anno, @dni, @sitlaboral, @apellidos, @nombres, @monto, @ncuota, @maxcuota)", _
    ...
    Public Sub ExportaDBFChulucanas(pData As DataSet, pNombreArchivoProceso As String)
        ...
        For Each ldr In pData.Tables(0).Rows
            ...
            'VALUES ( )
            Dim command As New OleDbCommand("INSERT INTO '" + pNombreArchivoProceso.Split(".")(0) +
            "' ( periodo, empresa, codmod, cargo, carben, t_plla, coddes, montodes, apepater, amater, nombre)" +
            " VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            connection)
            ' " VALUES ( @depe_id, @mes, @anno, @dni, @sitlaboral, @apellidos, @nombres, @monto, @ncuota, @maxcuota)", _
            command.Parameters.AddWithValue("periodo", ldr("periodo"))
    ...
    Public Sub ExportaDBFAMAZONAS(pData As DataSet, pNombreArchivoProceso As String)
        ...
        For Each ldr In pData.Tables(0).Rows
            ...
            'VALUES ( )
            Dim command As New OleDbCommand("INSERT INTO '" + pNombreArchivoProceso.Split(".")(0) +
            "' ( periodo, empresa, codmod, cargo, carben, t_plani, coddes, montodes, apepater, apemater, nombre, finicre)" +
            " VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            connection)
            ' " VALUES ( @depe_id, @mes, @anno, @dni, @sitlaboral, @apellidos, @nombres, @monto, @ncuota, @maxcuota)", _
            command.Parameters.AddWithValue("periodo", ldr("periodo"))
    ...
    Public Sub ExportaDBFUNFV(pData As DataSet, pNombreArchivoProceso As String)
        ...
        For Each ldr In pData.Tables(0).Rows
            Dim command As New OleDbCommand("INSERT INTO '" + pNombreArchivoProceso.Split(".")(0) +
            "' (CODVAR, CODPER, CLASE, APENOM, MONTO, CUOTA)" +
            " VALUES ( ?, ?, ?, ?, ?, ?)",
            connection)
