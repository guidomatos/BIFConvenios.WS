...
Public Class clsCuotaDO
    ...
    End Sub
    ' Methods
    Public Function FinalizaImportacionPagares(pstrCodigoProceso As String, pstrUsuario As String) As Integer
        Dim num As Integer
    ...
    Public Function InsertaDLENV(_drw As DataRow) As Integer
        Dim num As Integer
        Dim dasql As New DASQL
        Dim command As New SqlCommand
        Try
    ...
    Public Function InsertarHistoricoDLCCR(_drw As DataRow) As Object
        Dim obj2 As Object
        Dim dasql As New DASQL
        Dim command As New SqlCommand
        Try
    ...
    Public Function ObtenerDeudaDeIBS(pstrCodigoCliente As String, pstrAnio As String, pstrMes As String) As DataTable
    ...
    Public Function ObtenerPagareDeIBS(pstrCodigoCliente As String, pstrAnio As String, pstrMes As String) As DataTable
        Dim table2 As DataTable
        Dim connectionString As String = New DASQL().ConnectionAS400
        Dim connection As Connection = New ConnectionClass
        Dim adapter As New OleDbDataAdapter
        ...
        Try
            ...
            table3 = ObtenerTipoCuotaByCodigoIBS(pstrCodigoCliente)
            ...
            Dim str4 As String = Conversions.ToString(IIf((Len(Trim(table3.Rows(0)("DLEMEN").ToString)) = 1), ("0" & Trim(table3.Rows(0)("DLEMEN").ToString)), Trim(table3.Rows(0)("DLEMEN").ToString)))
            If Not ((num = 2) Or (num = 3)) Then
                str &= "                   (SELECT DLPPDD CONCAT '/' CONCAT DLPPDM CONCAT '/' CONCAT DLPPDY FROM DLPMT WHERE DLPACC = E.DLACC AND DLPPFL <> 'P' ORDER BY DLPPNU FETCH FIRST 1 ROW ONLY) AS FechaCargo,"
            Else
                strArray = New String() {str, "                   t.dlcxd1 concat '/'  concat ", str4, " concat '/' concat ", pstrAnio, " AS FechaCargo,"}
                str = String.Concat(strArray)
            End If
            str = Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject(((((((Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject((((((Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(((((str & "                      CASE WHEN t.DLCNC1 IS NULL THEN 0 ELSE t.DLCNC1 END as CuotasPactadas, (SELECT COUNT(*) FROM DLPMT WHERE DLPACC =  e.DLACC AND DLPPFL = 'P') AS CuotasPagadas, CASE WHEN (t.DLCNC1 - t.DLCCP1) IS NULL THEN 0 ELSE (t.DLCNC1 - (SELECT COUNT(*) FROM DLPMT WHERE DLPACC =  e.DLACC AND DLPPFL = 'P')) END as CuotasPendientes, " & ChrW(13) & ChrW(10) & "                       (CASE WHEN  (SELECT  COUNT(*) FROM PLPAD WHERE Plcnpg = E.DLACC) = 1 THEN (CASE WHEN (c.DLVCM  CONCAT c.DLVCA) = (SELECT  pldmpv CONCAT pldapv FROM PLPAD WHERE Plcnpg = E.DLACC ORDER BY pldmpv  DESC FETCH FIRST 1 ROW ONLY) THEN 1 ELSE 3 END) ELSE (CASE WHEN (c.DLVCM  CONCAT c.DLVCA) = (SELECT  pldmpv CONCAT pldapv FROM PLPAD WHERE Plcnpg = E.DLACC ORDER BY pldmpv  DESC FETCH FIRST 1 ROW ONLY) THEN 2 ELSE 3 END ) END) AS NroTipoCredito" & ChrW(13) & ChrW(10)) & "                      FROM DLCRE e INNER JOIN" & ChrW(13) & ChrW(10) & "                      DLCCR r ON e.DLACC = r.DLACC INNER JOIN" & ChrW(13) & ChrW(10)) & "                      DLCCR c ON e.DLACC = c.DLACC AND c.DLSTS = ''" & ChrW(13) & ChrW(10) & "                      INNER JOIN DLEMP D ON (D.DLECUN = E.DLCCC AND" & ChrW(13) & ChrW(10)) & "    (CAST (2000 + R.DLVCA AS CHARACTER (4))  CONCAT CASE WHEN LENGTH(TRIM(CAST(R.DLVCM AS CHARACTER (2) ))) = 1 THEN   '0' CONCAT TRIM(CAST(R.DLVCM AS CHARACTER ( 2 ))) ELSE   TRIM(CAST ( R.DLVCM AS CHARACTER ( 2 ) )) END <='" & pstrAnio), IIf((Strings.Len(Strings.Trim(pstrMes)) = 1), ("0" & Strings.Trim(pstrMes)), Strings.Trim(pstrMes))), "')) INNER JOIN DLCNT t ON e.DLACC = t.DLCACC"), ChrW(13) & ChrW(10))) & " WHERE     (TRIM(r.DLSTS) = '') ") & " AND CAST (2000 + R.DLVCA AS CHARACTER (4)) " & " CONCAT ") & " CASE WHEN LENGTH(TRIM(CAST(R.DLVCM AS CHARACTER (2) ))) = 1 THEN " & " " & ChrW(9) & "'0' CONCAT TRIM(CAST(R.DLVCM AS CHARACTER ( 2 )))") & " ELSE" & ChrW(9) & "TRIM(CAST ( R.DLVCM AS CHARACTER ( 2 ) ))") & " END <='" & pstrAnio), IIf((Strings.Len(Strings.Trim(pstrMes)) = 1), ("0" & Strings.Trim(pstrMes)), Strings.Trim(pstrMes))), "'")) & "GROUP BY e.DLCCC, e.DLAÑO, e.DLAGC, e.DLCOC, e.DLCCY, e.DLACC, e.DLCEM, e.DLNCL, e.DLAPP, e.DLAPM, e.DLPRN, e.DLSGN, e.DLCCR, e.DLPLA, " & ChrW(13) & ChrW(10) & "e.DLCUS, c.DLNCT, c.DLVCA, c.DLVCM, c.DLVCD, e.DLSTS, e.DLFLI, e.DLOAM,  t.DLCNC1, t.DLCCP1, t.dlcxd1, t.dlcxm1, t.dlcxy1, t.DLCNC1" & ChrW(13) & ChrW(10)) & " HAVING      (c.DLNCT = MAX(r.DLNCT)) AND e.DLCCC = " & pstrCodigoCliente) & " AND CAST ( 2000+c.DLVCA  AS CHARACTER ( 4 ) ) " & " CONCAT ") & " CASE WHEN LENGTH(TRIM(CAST ( c.DLVCM AS CHARACTER ( 2 ) ))) = 1 THEN" & " " & ChrW(9) & "'0' CONCAT TRIM(CAST ( c.DLVCM AS CHARACTER ( 2 ) ))") & " ELSE" & " " & ChrW(9) & "TRIM(CAST ( c.DLVCM AS CHARACTER ( 2 ) ))") & " END <='" & pstrAnio), IIf((Strings.Len(Strings.Trim(pstrMes)) = 1), ("0" & Strings.Trim(pstrMes)), Strings.Trim(pstrMes))), "'"))
            Dim aDODBRecordSet As Recordset = New RecordsetClass
            aDODBRecordSet = connection.Execute(str, Missing.Value, -1)
            aDODBRecordSet.ActiveConnection = Nothing
            connection.Close()
    ...
    Public Function ObtenerTipoCuotaByCodigoIBS(pstrCodigoIBS As String) As DataTable
